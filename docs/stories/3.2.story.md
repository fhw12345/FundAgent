...
- [ ] **AC1**: Active users are NOT redirected to login while using the app
- [ ] **AC2**: Access token is refreshed automatically before expiry
- [ ] **AC3**: Refresh happens silently (no UI interruption)
- [ ] **AC4**: Failed refresh (e.g., refresh token expired) gracefully redirects to login with message
- [ ] **AC5**: User activity (clicks, API calls) extends session
- [ ] **AC6**: Idle timeout only triggers after X minutes of NO activity (configurable)
...
- [ ] **Task 5**: Add 401 interceptor with refresh retry logic (already exists in frontend)
- [ ] **Task 6**: Add user-friendly session expiry handling (nice-to-have)
- [ ] **Task 7**: Test scenarios: active use, idle, expired refresh token
- [ ] **Task 8**: Add logging for auth events (debug production issues)
...
1. **Root Cause**: `AuthService.ACCESS_TOKEN_EXPIRE_MINUTES` was set to 7 days (10,080 min) while `TokenService.ACCESS_TOKEN_EXPIRE_MINUTES` was 30 minutes. This caused tokens created at login to have different expiry than tokens created by refresh.

2. **Fix Applied**: Changed `AuthService.ACCESS_TOKEN_EXPIRE_MINUTES` from `60 * 24 * 7` (7 days) to `30` (30 minutes) to match `TokenService`.

3. **Existing Frontend Refresh Logic**: The frontend already has proper token refresh mechanisms:
   - Proactive refresh (5 min before expiry) in `tokenRefresh.ts`
   - Reactive refresh (401 interceptor) in `api.ts`
   - Token rotation on refresh (backend)

4. **Test Updated**: `test_token_expiry_configuration` updated to expect 30 minutes.
...
# 941 passed, 73 warnings

# Lint passes
ruff check src/services/auth_service.py tests/test_auth_service.py
# All checks passed!
```
