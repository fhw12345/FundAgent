...
**Superseded** → See [Story 2.8](./2.8.story.md) for final implementation

> **Note**: Story 2.8 implemented the Put/Call Ratio as a reusable service with AI tool integration, which supersedes this story's scope.
...
|-----------|------------------|------------------|
| < 0.5 | **Extreme Bullishness** | 🔴 HIGH RISK - Complacency, potential top |
| 0.5 - 0.7 | **Bullish** | 🟡 ELEVATED - One-sided positioning |
| 0.7 - 1.0 | **Neutral** | 🔵 NORMAL - Balanced market |
| 1.0 - 1.3 | **Bearish** | 🟢 LOW RISK - Fear present, potential bottom |
| > 1.3 | **Extreme Bearishness** | 🟢 VERY LOW - Capitulation, often bullish |

#### Why It's Contrarian

The put/call ratio works as a **contrarian indicator** because:

1. **Crowd Psychology**: When everyone is bullish (low PCR), there's no one left to buy → prices struggle to rise
2. **Hedging Signal**: Low PCR means institutions aren't hedging → complacency before corrections
...
| **Moneyness bias** | Deep ITM/OTM distort sentiment | Focuses on ±15% of spot price |
...
```
Notional = Open Interest × Option Price × 100 (contract multiplier)

Dollar-Weighted PCR = Σ(Put Notionals) / Σ(Call Notionals)
```

### ATM Zone Definition

**ATM Zone** = Options with strikes within ±15% of current stock price

```python
def is_atm_zone(strike: float, current_price: float) -> bool:
    """Check if option is in ATM zone (±15% of current price)."""
...
|--------|-----------|-----------|
| ATM Zone | Strike ±15% of spot | Exclude lottery tickets |
| Min Premium | ≥ $0.50 | Exclude penny options |
| Min Open Interest | ≥ 500 contracts | Ensure liquidity |
...
def normalize_pcr_to_score(pcr: float) -> float:
    ...
    - PCR 0.3 (extreme bullish complacency) → Score 95 (high risk)
    - PCR 0.7 (neutral) → Score 50 (normal)
    - PCR 1.3 (extreme bearish fear) → Score 5 (low risk)
    ...
    # Linear mapping: 0.3 → 95, 1.15 → 50, 2.0 → 5
...
async def _calculate_options_put_call_ratio(
    ...
    1. For each symbol in top 5 AI basket:
       ...
       c. Filter to ATM zone (±15% of current price)
...
- [ ] **Task 2: Cache Keys** (AC: 9)
  - [ ] 2.1 Add `CacheKeys.quote(symbol)` → `market:quote:NVDA`
  - [ ] 2.2 Add `CacheKeys.options(symbol)` → `market:options:NVDA`
...
- [ ] **Task 5: Put/Call Ratio Metric Implementation** (AC: 1, 3, 4, 5)
  ...
  - [ ] 5.2 Implement ATM zone filtering (±15% of current price)
  - [ ] 5.3 Implement quality filters (min premium $0.50, min OI 500)
  - [ ] 5.4 Calculate dollar-weighted PCR
  - [ ] 5.5 Implement score normalization (inverted PCR → 0-100)
