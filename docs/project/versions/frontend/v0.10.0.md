...
- **Color-Coded Modal Sections**:
  - 🤖 **Agent Mode** (Blue gradient: `from-blue-50 to-blue-100`)
    - Flow diagram: Query → Analyze → Auto-select Tools → Insights
    - Visual arrows and step indicators
  - 💬 **Copilot Mode** (Purple gradient: `from-purple-50 to-purple-100`)
    - Flow diagram: Click Analysis → View Chart → AI Explains
    - Manual control with AI guidance
  - 📊 **Portfolio Agent** (Green gradient: `from-green-50 to-green-100`)
    - 4-step guide: Navigate → View Holdings → Check Transactions → Analyze P&L
...
- **Visual Status Indicators**:
  - 🔄 Animated spinner for running tools (using Lucide's Loader2 with spin animation)
  - ✅ Green checkmark for successful completion (CheckCircle icon)
  - ❌ Red X for failed tools (XCircle icon)

- **Rich Metadata Display**:
  - Tool icon and display name (e.g., 📈 "Market Movers")
...
#    - Shows "📈 Market Movers" with "Running" status
...
┌─────────────────────────────────────────────┐
│ ✅ Market Movers         [AAPL]  883ms      │
│ ▼ View Output                                │ ← Expandable output
└─────────────────────────────────────────────┘
```
