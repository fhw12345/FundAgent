...
const convertTimezone = (easternTimeStr: string, targetTimezone: SupportedTimezone): Date => {
...
export const useChartData = (
    ...
    const convertToChartData = useCallback(() => {
        ...
        const convertedData = data.map(point => ({
            ...
            if (typeof a.convertedTime === 'number' && typeof b.convertedTime === 'number') {
                return a.convertedTime - b.convertedTime
            }
            return a.convertedTime.toString().localeCompare(b.convertedTime.toString())
        })
