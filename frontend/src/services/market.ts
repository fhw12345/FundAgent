...
export const marketService = {
  ...
  async searchSymbols(query: string): Promise<SymbolSearchResponse> {
    const response = await apiClient.get<SymbolSearchResponse>(`/api/market/search?q=${encodeURIComponent(query)}`)
  ...
  async getPriceData(
    ...
    try {
      const response = await apiClient.get<PriceDataResponse>(`/api/market/price/${symbol}?${params}`)
      return response.data
    } catch (err: any) {
      if (err?.response?.data?.detail && typeof err.response.data.detail === 'object') {
        const d = err.response.data.detail
        throw { message: d.message || 'Price data fetch failed', suggestions: d.suggestions || [], original: err }
      }
      throw err
    }
  },
  ...
  async getSymbolInfo(symbol: string): Promise<SymbolInfo> {
    const response = await apiClient.get<SymbolInfo>(`/api/market/info/${symbol}`)
    return response.data
  },
   ...
   */
  convertToChartData(priceData: PriceDataPoint[], chartType: 'line' | 'candlestick' = 'line') {
    if (chartType === 'line') {
  ...
  createDebouncedSearch(delay: number = 300) {
    ...
    return (query: string, callback: (results: SymbolSearchResponse) => void) => {
