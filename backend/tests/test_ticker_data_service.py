...
class TestTickerDataService:
    ...
    def test_validate_parameters_period_and_dates_conflict(self, ticker_service):
        ...
        with pytest.raises(ValueError, match="Cannot specify both 'period' and date range"):
    ...
    def test_normalize_to_date_range_with_period(self, ticker_service):
        ...
        with patch('src.core.data.ticker_data_service.DateUtils.period_to_date_range') as mock_convert:
    ...
    def test_calculate_ttl_current_vs_historical(self, ticker_service):
        ...
        # Historical data
        historical_ttl = ticker_service._calculate_ttl(interval, "2024-01-01", "2024-06-01")
    ...
    async def test_integration_cache_key_consistency(self, ticker_service):
        ...
        with patch('src.core.data.ticker_data_service.DateUtils.period_to_date_range') as mock_convert:
            ...
            await ticker_service.get_ticker_history("AAPL",
                                                  interval="1d",
    ...
    def test_cache_key_uniqueness(self, ticker_service):
        ...
        # Different symbol
        key1 = ticker_service._generate_cache_key("MSFT", "2024-01-01", "2024-06-01", "1d")
        assert key1 != base_key

        # Different dates
        key2 = ticker_service._generate_cache_key("AAPL", "2024-02-01", "2024-06-01", "1d")
        assert key2 != base_key

        # Different interval
        key3 = ticker_service._generate_cache_key("AAPL", "2024-01-01", "2024-06-01", "1h")
...
class TestTickerDataServiceCaching:
    ...
    async def test_cache_miss_returns_empty_df(self, ticker_service, mock_redis_cache):
        ...
        with patch('yfinance.Ticker') as mock_ticker_class:
            ...
            result = await ticker_service.get_ticker_history("AAPL", interval="1d", period="6mo")
    ...
    @pytest.mark.asyncio
    async def test_cache_hit_returns_cached_data(self, ticker_service, mock_redis_cache):
...
class TestTickerDataServiceYfinance:
    ...
    @pytest.mark.asyncio
    async def test_fetch_from_yfinance_empty_response(self, ticker_service, mock_redis_cache):
    ...
    @pytest.mark.asyncio
    async def test_fetch_from_yfinance_exception(self, ticker_service, mock_redis_cache):
    ...
    @pytest.mark.asyncio
    async def test_yfinance_uses_start_end_dates(self, ticker_service, mock_redis_cache):
