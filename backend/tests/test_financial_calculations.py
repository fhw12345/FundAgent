...
class TestDividendYieldCalculation:
    ...
    @pytest.mark.asyncio
    async def test_dividend_yield_decimal_to_percentage_conversion(
        self, stock_analyzer, mock_yfinance_info
    ):
        """
        ...
        with patch("yfinance.Ticker") as mock_ticker:
            ...
            mock_instance.history.return_value = pd.DataFrame(
                {"Close": [170.0, 175.50], "Volume": [50000000, 45000000]},
            ...
            assert (
                abs(result.dividend_yield - 0.47) < 0.01
            ), f"Expected 0.47%, got {result.dividend_yield}%"
            assert (
                0 < result.dividend_yield < 10
            ), f"Dividend yield {result.dividend_yield}% is unreasonable."
        ...
        for yfinance_value, expected_result, description in test_cases:
            with patch("yfinance.Ticker") as mock_ticker:
                ...
                if expected_result is None:
                    ...
                    assert (
                        abs(result.dividend_yield - expected_result) < 0.01
    ...
    async def test_dividend_yield_integration_format(self, stock_analyzer):
        ...
        with patch("yfinance.Ticker") as mock_ticker:
            ...
            assert (
                abs(fundamentals.dividend_yield - 0.47) < 0.01
            ), f"Expected 0.47%, got {fundamentals.dividend_yield}%"
            assert (
                0.1 < fundamentals.dividend_yield < 1.0
            ), "For AAPL, dividend yield should be realistic"
