...
class AlphaVantageMarketDataService:
    ...
    async def get_news_sentiment(
        ...
        try:
            ...
            if "feed" not in data:
                sanitized = self._sanitize_response(data)
                logger.warning("No news sentiment data", tickers=tickers, response=sanitized)
                return {"feed": [], "sentiment_score_definition": data.get("sentiment_score_definition")}
    ...
    async def get_top_gainers_losers(self) -> dict[str, Any]:
        ...
        try:
            ...
            if not any(key in data for key in ["top_gainers", "top_losers", "most_actively_traded"]):
                sanitized = self._sanitize_response(data)
